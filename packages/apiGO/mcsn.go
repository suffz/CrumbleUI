...
func CheckChange(bearer string, PS *h2.ProxyAuth) bool {
	if request, err := h2.BuildRequest("https://api.minecraftservices.com/minecraft/profile/namechange", "GET", "", map[string]string{"Authorization": "Bearer " + bearer}, PS); err == nil {
		_, data, err := request.Commit()
		if err != nil {
			return false
		}
		...
		case 200:
			if data.Body != nil {
				resp, _ := io.ReadAll(data.Body)
				var Data struct {
					NC bool `json:"nameChangeAllowed"`
				}
				io.ReadAll(data.Body)
